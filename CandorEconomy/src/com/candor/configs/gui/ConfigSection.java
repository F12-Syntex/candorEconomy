...
import com.candor.utils.Communication;
import com.candor.utils.ComponentBuilder;
import com.candor.utils.Input;
...
public class ConfigSection extends ConfigGUI {
	...
	public List<PagedItem> Contents() {
		...
		this.configSection.getKeys(false).forEach(i -> {
			...
			ItemStack section = new ItemStack(Material.valueOf(Candor.getInstance().configManager.configs.Sections));
			ItemStack ints = new ItemStack(Material.valueOf(Candor.getInstance().configManager.configs.Numbers));
			ItemStack text = new ItemStack(Material.valueOf(Candor.getInstance().configManager.configs.Text));
			ItemStack booleanON = new ItemStack(Material.valueOf(Candor.getInstance().configManager.configs.BooleanON));
			ItemStack booleanOFF = new ItemStack(Material.valueOf(Candor.getInstance().configManager.configs.BooleanOFF));
			ItemStack lists = new ItemStack(Material.valueOf(Candor.getInstance().configManager.configs.Lists));
			...
			if(this.configSection.isConfigurationSection(i)) {
				...
				ItemStack item = ItemGenerator.getItem("&7" + i, section, ComponentBuilder.createLore("&5This item cannot be modified!"));
				Runnable execution = () -> {
					
				};
				
				if(obj instanceof String) {
					item = ItemGenerator.getItem("&7" + i, text, ComponentBuilder.createLore("&6Value: &e" + obj.toString(), "&cClick here to change this value."));
					execution = () -> {
						...
						Communication.applyInput(player, new Input() {
							...
							public void onRecieve(String o) {
								...
								Bukkit.getScheduler().callSyncMethod(Candor.getInstance(), new Callable<Object>() {
									@Override
				...
				if(obj instanceof Boolean) {
					...
					if(value) {
						item = ItemGenerator.getItem("&7" + i, booleanON, ComponentBuilder.createLore("&6Click here to change this value to &efalse."));	
						execution = () -> {
							player.closeInventory();
						...
						item = ItemGenerator.getItem("&7" + i, booleanOFF, ComponentBuilder.createLore("&6Click here to change this value to &etrue."));	
						execution = () -> {
							player.closeInventory();
				...
				if(obj instanceof Integer) {
					item = ItemGenerator.getItem("&7" + i, ints, ComponentBuilder.createLore("&6Value: &e" + obj.toString(), "&cClick here to change this value."));
					execution = () -> {
